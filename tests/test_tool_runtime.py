...
@pytest.mark.asyncio
async def test_runtime_executes_only_allowlisted_tools():
    ...
    result = await runtime.execute([ToolCall("calculator", {"expression": "2+2"})])
    assert result[0].success is True
    assert result[0].output == "2+2"


@pytest.mark.asyncio
async def test_runtime_rejects_non_allowlisted_tool():
    registry = ToolRegistry()
    registry.register("calculator", "safe arithmetic", lambda expression: expression)
...
@pytest.mark.asyncio
async def test_runtime_stops_after_first_tool_failure():
    registry = ToolRegistry()
    ...
    results = await runtime.execute([ToolCall("first", {}), ToolCall("second", {})])
    assert len(results) == 1
    assert results[0].success is False


@pytest.mark.asyncio
async def test_bounded_agent_enforces_step_budget():
    registry = ToolRegistry()
...
@pytest.mark.asyncio
async def test_iot_tools_remain_disabled_by_default():
    registry = ToolRegistry()
    ...
    result = await runtime.execute([ToolCall("iot_write", {})])
    assert result[0].success is False
